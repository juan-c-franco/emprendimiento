                        ...
                        if (!validationError) {
                            if (args === arg) {
                                ...
                                guardaProgramacion(moment(horaInicio).format('YYYY-MM-DD kk:mm'),
                                        moment(horaFin).format('YYYY-MM-DD kk:mm'), $("#descripSensibilizacion").val(),
                                        $("#maximoAsistentes").val(), $("#ubicacionSensibilizacion").val());
...
function guardaProgramacion(fechaInicial, fechaFin, descripcion, maxCantidad, ubicacion) {
    if (!verificarHoras())
        return false;
    jQuery.ajax({
        method: "POST",
        ...
        data: {descripcion: descripcion, fechaInicial: fechaInicial.toString(),
            fechaFinal: fechaFin.toString(), maxCantidad: maxCantidad,
        ...
        success: function (result, status, xhr) {
            if (result.status == "1") {
                ...
                swal({
                    ...
                    $(".preloader").fadeIn();
                    loadView('Programar Sesión', context + '/programarSesiones');
                });
            } else {
                swal({
                    title: "Programar Sesión",
        ...
        success: function (result, status, xhr) {
            if (result.status == "1") {
                ...
                result.forEach(function (item, index, array) {
                    ...
                    if (item.estadosesion.idestadosesion == 3) {
                        ...
                        jQuery.ajax({
                            ...
                            success: function (result, status, xhr) {
                                if (new Date(array[index].fechainicio) < new Date()
                                        && item.estadosesion.idestadosesion == 1
                                        && result.asistenciaDTO && result.asistenciaDTO.length > 0) {
                                    sesion.title = "Registrar Asistencia";
                                    sesion.className = 'bg-registrar';
                                } else if (new Date(array[index].fechainicio) < new Date()
                                        && item.estadosesion.idestadosesion == 1
                                        && result.asistenciaDTO && result.asistenciaDTO.length == 0) {
                                    sesion.title = "Culminada";
                                    sesion.className = 'bg-culminada';
