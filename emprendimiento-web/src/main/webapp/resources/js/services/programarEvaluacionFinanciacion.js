                        ...
                        if (!validationError) {
                            if (args === arg) {
                                ...
                                guardaProgramacion(moment(horaInicio).format('YYYY-MM-DD kk:mm'),
                                        moment(horaFin).format('YYYY-MM-DD kk:mm'), $("#descripFinanciacion").val(),
                                        $("#ubicacionFinanciacion").val());
                                calendar.unselect();
                            }
                        }
        ...
        success: function (result, status, xhr) {
            if (result.status == "1") {
                ...
                swal({
                    ...
                    $(".preloader").fadeIn();
                    loadView('Programar Sesión', context + '/programarEvaluacionFinanciacion');
                });
            } else {
                swal({
                    title: "Programar Sesión",
        ...
        success: function (result, status, xhr) {
            ...
            if (result.status = "1") {
                swal({
                    ...
                    $("#modalOpciones").modal("hide");
                    loadView('Programar Evalaución', context + '/programarEvaluacionFinanciacion');
                });
            } else {
                swal({
                    title: "Programar Sesión",
...
function registrarAsistencia() {
    swal({
        ...
        if (isConfirm) {
            ...
            setTimeout(function () {
                $(".preloader").fadeIn();
                $("#body_content").load(context + '/traerAsistentesE',
