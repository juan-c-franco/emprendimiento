...
function registrarCapacitacion() {
    ...
    $(".preloader").fadeIn();
    $.post(context + "/crearCapacitacionPrograma", {nombrecapacitacionprograma: nombreCapacitacion,
        idoferenteinstitucion: idOferenteInstitucion, idcategoria: categorias}, function (data) {
        $(".preloader").fadeOut();
        ...
        if (data.status === '1') {
            ...
            swal({
                title: "Error Registrando Capacitación",
                text: data.descripcion,
                type: "error",
                confirmButtonText: "Ok",
                showCancelButton: false
